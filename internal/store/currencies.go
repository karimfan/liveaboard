...
// Currency is one entry in the ISO 4217 currency catalog the app
// recognises for organization currency and accepted-currencies pickers.
// Exponent follows ISO 4217 conventions (USD=2, JPY=0, BHD=3) except
// where the existing codebase had pre-Sprint-023 values; those are
// preserved for backwards compatibility (see exponentOverrides).
...
// exponentOverrides records intentional deviations from the ISO 4217
// default for codes the app had to ship before this catalog existed.
// Keep this list narrow — every entry trades wire-format stability
// against external-spec compliance.
var exponentOverrides = map[string]int{
	// IDR shipped with exponent 0 in early sprints (the rupiah's sen
	// subdivision is dormant). Changing it would re-interpret stored
	// settlement_total_minor on every closed folio.
	"IDR": 0,
}

// iso4217 is the curated catalog of active ISO 4217 currencies the app
// will offer in pickers. The order matters: pickers preserve catalog
// order as a fallback when there is no search query.
var iso4217 = []Currency{
	...
	{"IDR", "Indonesian Rupiah", 2}, // overridden to 0 below for back-compat
...
// currencyExponents is the lookup the rest of the app already uses.
// It is derived from iso4217 + exponentOverrides at package init.
var currencyExponents = func() map[string]int {
	m := make(map[string]int, len(iso4217))
	for _, c := range iso4217 {
		m[c.Code] = c.Exponent
	}
	for code, exp := range exponentOverrides {
		m[code] = exp
	}
	return m
}()

// AllCurrencies returns the catalog with any exponent overrides applied,
// in canonical (alphabetical-by-code) order. Used by the HTTP currency
// catalog endpoint that drives the frontend pickers.
func AllCurrencies() []Currency {
	out := make([]Currency, len(iso4217))
	for i, c := range iso4217 {
		if exp, ok := exponentOverrides[c.Code]; ok {
			c.Exponent = exp
		}
		out[i] = c
	}
