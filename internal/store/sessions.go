...
// SessionByTokenHash returns an active (non-expired, non-revoked) session
// and bumps last_seen_at, or ErrNotFound.
...
// DeleteSessionByTokenHash removes a single session.
...
// DeleteSessionsForUser removes every session for a user. Used by
// password reset, change-email confirmation, and account deactivation.
...
// DeleteSessionsForUserExcept deletes every session for a user EXCEPT
// the one identified by tokenHash. Used by change-password and
// change-email confirmation: invalidate other devices but keep the
// caller signed in.
func (p *Pool) DeleteSessionsForUserExcept(ctx context.Context, userID uuid.UUID, keepTokenHash []byte) error {
	_, err := p.Exec(ctx,
